...
func _ready() -> void:
	...
	_original_triangle_scale = notice_triangle_sprite.scale
...
func _physics_process(delta) -> void:
	...
	distance_to_player = _entity.global_position.distance_to(
		_player.global_position
	)
	
	# only make triangle visible if can be seen by camera
	if _camera.is_position_in_frustum(global_position):
		notice_triangle_sprite.position = _camera.unproject_position(global_position)
