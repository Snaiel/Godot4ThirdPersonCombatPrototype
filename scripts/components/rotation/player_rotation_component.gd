...
func _physics_process(delta: float) -> void:
	...
	if rotate_towards_target and target:
		...
		if delta and not target:
			...
			if player.is_on_floor():
				...
				if abs(_last_input_on_ground.x) > 0.2 and abs(_last_input_on_ground.z) < 0.2:
					# if moving left or right relative to camera before jumping,
					# slow down opposite sidewards movement mid air
					if is_equal_approx(_input_direction.x, -_last_input_on_ground.x):
						move_direction.x = _input_direction.normalized().x * 0.2
				else:
					# reduce side to side movement mid air
					move_direction.x = move_direction.x * 0.6
					# if moving forwards or backwards relative to camera before jumping,
					# slow down opposite movement mid air
					if is_equal_approx(_input_direction.z, -_last_input_on_ground.z) and abs(_input_direction.z) > 0.2:
						move_direction.z = _input_direction.normalized().z * 0.2
	...
	if _freelook_turn and _turn_all_the_way and \
		...
		not (not player.is_on_floor() and _last_input_on_ground.length() < 0.1):
